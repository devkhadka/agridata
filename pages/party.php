...
<div id='heading'><h3 style='float:left'>View Party</h3><a href='?page=addparty'><div id='linking'>Add Party</div></a><br/><br/></div>
<table id='list'></table> 
<div id='pager'></div> 
";
...
//        $args = array("name"=>$_POST['name'],"address"=>$_POST['address'],"phone"=>$_POST['phone'] , "MR_id"=>$_POST["MR_id"]);
    ...
    $str .="<div id='heading'>
            <h3 style='float:left'>Add Party</h3><a href='?page=viewparty'><div id='linking'>Manage Party</div></a><br/><br/>
    </div>
        <div id='sub-content'><form method='POST' action='{$_SERVER['REQUEST_URI']}'>
        <div class='form_row'><div class='label'>Name :</div><div class='act_obj'> <input size='25' type='text' name='name' value=\"{$form->value('name')}\" /></div><div class='err_msg'>{$form->error('name')}</div></div>
        <div class='form_row'><div class='label'>Address :</div><div class='act_obj'> <input type='text' size='25' name='address' value=\"{$form->value('address')}\" /></div><div class='err_msg'>{$form->error('address')}</div></div>
        <div class='form_row'><div class='label'>Phone :</div><div class='act_obj'> <input type='text' size='25' name='phone' value=\"{$form->value('phone')}\" /></div><div class='err_msg'>{$form->error('address')}</div></div>";
    $str.=" <div class='form_row'><div class='label'>Marketing Representative:</div><div class='act_obj'> <select name='MR_id[]' style='width:125px' multiple> ";

    foreach($MRList as $count => $MR){
        $str.= "<option value='{$MR['id']}'>{$MR['username']}</option>";
    }

    $str.="</select></div></div>";
    $str.="<div><input type='hidden' name='add'></div>
        <div>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp;&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp;<input type='submit' name='submit' value='Add'></div>
        </form></div>";
...
//        die();
        $str .="<div id='heading'><h3>Edit Party</h3></div>
            ...
            <div class='form_row'><div class='label'>Name :</div><div class='act_obj'> <input type='text' name='name' size='40' value=\"{$form->value('name')}\" /></div><div class='err_msg'>{$form->error('name')}</div></div>
            <div class='form_row'><div class='label'>Address :</div><div class='act_obj'> <input type='text'  size='40' name='address' value=\"{$form->value('address')}\" /></div><div class='err_msg'>{$form->error('address')}</div></div>
            <div class='form_row'><div class='label'>Phone :</div><div class='act_obj'> <input type='text'  size='40' name='phone' value=\"{$form->value('phone')}\" /></div><div class='err_msg'>{$form->error('address')}</div></div>";
        $str.=" <div class='form_row'><div class='label'>Marketing Representative:</div><div class='act_obj'> <select name='MR_id[]' multiple style='width:225px'> ";
        $selected = "";

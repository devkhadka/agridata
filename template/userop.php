...
function addUser(){
    ...
    $ret="<div id ='heading'> <h3>Add User</h3> </div>
        <div id='content' >"; 
...
<table align="left" border="0" cellspacing="0" cellpadding="3">
        <tr>
                <td>Name:</td>
                <td><input type="text" name="name" maxlength="30" value="{$form->value("name")}"></td>
                <td>{$form->error("name")}</td>
        </tr>
        <tr>
                <td>Username:</td>
                <td><input type="text" name="username" maxlength="30" value="{$form->value("username")}"></td>
        ...
        <tr>
                <td>Phone No:</td>
                <td><input type="text" name="phone" maxlength="30" value="{$form->value("phone")}"></td>
                <td>{$form->error("phone")}</td>
        </tr>
        <tr>
                <td>Address Line 1:</td>
                <td><input type="text" name="addl1" maxlength="30" value="{$form->value("addl1")}"></td>
                <td>{$form->error("addl1")}</td>
        </tr>
        <tr>
                <td>Address Line 2:</td>
                <td><input type="text" name="addl2" maxlength="30" value="{$form->value("addl2")}"></td>
                <td>{$form->error("addl2")}</td>
        </tr>
        <tr>
                <td colspan="2" align="right"><input type="hidden" name="adduser" value="1"><input type="submit" value="Add "></td>
        </tr>
</table>
</form>
</div>
...
function changePassword(){
    ...
    $ret.=<<<EOT
        ...
        <td><input type="password" name="curpass" maxlength="30" value="{$form->value("curpass")}"></td>
        <td>{$form->error("curpass")}</td>
        </tr>
        <tr>
        <td>New Password:</td>
        <td><input type="password" name="newpass" maxlength="30" value="{$form->value("newpass")}"></td>
        <td>{$form->error("newpass")}</td>
        </tr>
        <tr>
        <td>Conform Password:</td>
        <td><input type="password" name="newpass2" maxlength="30" value="{$form->value("newpass2")}"></td>
        <td></td>
        </tr>
